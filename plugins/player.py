...
from utils import download, get_admins, is_admin, get_buttons, get_link, import_play_list, leave_call, play, \
...
@Client.on_message(
    filters.command(["play", f"play@{Config.BOT_USERNAME}"]) & (filters.chat(Config.CHAT) | filters.private))
...
@Client.on_message(filters.command(["shuffle", f"shuffle@{Config.BOT_USERNAME}"]) & admin_filter & (
        filters.chat(Config.CHAT) | filters.private))
async def shuffle_play_list(client, m: Message):
    if not Config.CALL_STATUS:
        msg = await m.reply("Not joined any voicechat.")
        await asyncio.sleep(2)
        await msg.delete()
        await m.delete()
        return
...
@Client.on_message(filters.command(["clearplaylist", f"clearplaylist@{Config.BOT_USERNAME}"]) & admin_filter & (
        filters.chat(Config.CHAT) | filters.private))
...
@Client.on_message(filters.command(["yplay", f"yplay@{Config.BOT_USERNAME}"]) & admin_filter & (
        filters.chat(Config.CHAT) | filters.private))
...
@Client.on_message(filters.command(["stream", f"stream@{Config.BOT_USERNAME}"]) & admin_filter & (
        filters.chat(Config.CHAT) | filters.private))
async def stream(client, m: Message):
    ...
    if match:
        ...
        stream_link = link
    msg = await m.reply(f"[Streaming]({stream_link}) Started.", disable_web_page_preview=True)
...
async def notforu(_, m: Message):
    await _.send_cached_media(chat_id=m.chat.id,
...
async def not_chat(_, m: Message):
    buttons = [
        [
            InlineKeyboardButton('⚡️Make Own Bot', url='https://github.com/jinspalakkattu/VCPlayerBot'),
            InlineKeyboardButton('🧩 Join Here', url='https://t.me/joinchat/7qlEga5lO0o2MTg0'),
        ]
    ]
    await m.reply(
        "<b>You can't use this bot in this group, for that you have to make your own bot from the [SOURCE CODE]("
        "https://github.com/jinspalakkattu/VCPlayerBot) below.</b>",
